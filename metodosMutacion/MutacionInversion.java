	...
	public void mutacion(Cromosoma[] pob, double probMutacion, int tamPob) {
		...
		for (int i = 0; i < tamPob; i++) {
			...
			if (prob < probMutacion){	// mutan los genes con prob<probMutacion
				pos1 = (int) (Math.random() * (pob[i].getnGenes - 1)); //0 incluido y ngenes-1 no(ultima pos no incluida)
				pos2 = (int) (Math.random() * (pob[i].getnGenes) + (pos1 + 1)); //pos1+1 incluido y ngenes no (ultima pos incluida)
